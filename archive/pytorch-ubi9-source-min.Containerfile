...
RUN --mount=type=cache,id=cache,dst=/root/.cache,mode=0777,Z \
    ...
    cd /workspace/audio && \
    PYTORCH_VERSION=$(${VIRTUAL_ENV}/bin/${PYTHON} -m pip show torch | grep 'Version:' | awk '{ print $2; }') USE_CUDA=1 TORCH_CUDA_ARCH_LIST="7.0 8.0 8.6 9.0" BUILD_SOX=0 MAX_JOBS=$(nproc) ${VIRTUAL_ENV}/bin/${PYTHON} setup.py bdist_wheel
...
ENV LD_LIBRARY_PATH="${CUDA_HOME}/lib64:${INTEL_MKL_HOME}/lib:${PYTORCH_HOME}/lib:${PYTORCH_AUDIO_HOME}/lib:${PYTORCH_VISION_HOME}:${LD_LIBRARY_PATH}"
